...
def create_cumprod_model(pb_file_path, input_size):
    ''' Create tensorflow model for cumprod op.
    '''
    with tf.Session(graph=tf.Graph()) as sess:
        x = tf.placeholder(tf.float32, shape=input_size, name='X')

        y = tf.math.cumprod(x, axis=1, exclusive=True, reverse=True, name='Y')
...
# Create model
create_cumprod_model(model_path, input_shape)
