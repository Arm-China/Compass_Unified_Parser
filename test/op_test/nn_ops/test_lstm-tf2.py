...
def create_lstm_model(model_path, x1_size, time_major, return_sequences, go_backwards):
...
for time_major in (True, False):
    for return_sequences in (False, True):
        for go_backwards in (True, False):
            model_name = '-'.join([TEST_NAME, str(time_major), str(return_sequences), str(go_backwards)])
            model_path = model_name + '.h5'
            # Create model
            create_lstm_model(model_path, x1_shape, time_major, return_sequences, go_backwards)
