...
def create_gru_model(model_path, x1_size, initial_state, time_major, reset_after):
...
for time_major in (True, False):
    for reset_after in (True, False):  # gruv1: reset_after=True; gruv3: reset_after=False
        ...
        model_name = '-'.join([TEST_NAME, str(time_major), str(reset_after)])
        model_path = model_name + '.h5'
        # Create model
        create_gru_model(model_path, x1_shape, initial_state, time_major, reset_after)
