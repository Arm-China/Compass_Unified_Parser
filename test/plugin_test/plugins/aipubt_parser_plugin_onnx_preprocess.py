...
class OnnxPreprocess(ParserOp):
    op_type = 'MyPreprocess'
    priority = 50
    input_nodes = ['ONNX_INPUT']  # input node names to insert preprocess
    input_shapes = [[1, 4, 300, 300]]  # new input shapes after adding preprocess

    def __init__(self, fw, attrs):
        super(OnnxPreprocess, self).__init__(fw, attrs)
